...
# --- CONFIGURATION ---
st.set_page_config(page_title="Expert WMS Pro v9.6 - Master Logistics", layout="wide")

if 'db_refs' not in st.session_state:
    st.session_state.db_refs = pd.DataFrame([{"Référence":"PRODUIT_TEST", "L":300, "W":200, "H":150, "P":15.0}])

# --- MOTEUR D'OPTIMISATION TETRIS (VALIDÉ) ---
...
def draw_real_pallet(fig, x_off, w_p, l_p, z_off, color="#8D6E63"):
    """Palette avec structure dés/semelles détaillée"""
...
# --- UI ---
st.sidebar.title("🏭 Expert WMS v9.6")
fmt_pal = st.sidebar.selectbox("Format Palette", ["800x1200 (EURO)", "1000x1200 (VMF)"])
w_p = 800 if "800" in fmt_pal else 1000

l_lisse = st.sidebar.selectbox("Longueur Lisse (mm)", [2700, 3600, 1350], index=0)
capa_lisse = st.sidebar.number_input("Capacité Lisse (kg)", value=3000)
h_rack_max = st.sidebar.number_input("Hauteur Utile (mm)", value=1800)
...
nb_pal_possibles = l_lisse // w_p
poids_total_lisse = poids_pal * nb_pal_possibles
espace_perdu = l_lisse - (nb_pal_possibles * w_p)

# --- DASHBOARD ---
st.header(f"Simulation Logistique : {ref_sel} sur Lisse {l_lisse}")
kpi1, kpi2, kpi3, kpi4 = st.columns(4)
kpi1.metric("Colis / Palette", len(plan_a) * nb_c)
kpi2.metric("Charge Totale Lisse", f"{poids_total_lisse} kg")
kpi3.metric("Espace Vide Lisse", f"{espace_perdu} mm")
kpi4.metric("Garde d'Air Haut", f"{h_rack_max - (150 + nb_c * item['H'])} mm")

if poids_total_lisse > capa_lisse:
    st.error(f"🛑 ALERTE CHARGE : La lisse est en surcharge de {poids_total_lisse - capa_lisse} kg !")

tabs = st.tabs(["🏗️ Implantation Rack & 3D", "📋 Plans de Pose 2D"])

with tabs[0]:
    v1, v2 = st.columns(2)
    with v1:
        st.subheader("Palette Optimisée (Croisée)")
        f1 = go.Figure()
        ...
        f1.update_layout(scene=dict(aspectmode='data'), height=650, margin=dict(l=0,r=0,b=0,t=0))
        st.plotly_chart(f1, use_container_width=True)
        
    with v2:
        st.subheader("Vue Alvéole (Positionnement Extrémités)")
        f2 = go.Figure()
        # Structure Rack
        ...
        draw_box(f2, 0, l_lisse, 0, 100, -120, 0, lisse_col) # Lisse AV
        draw_box(f2, 0, l_lisse, 1100, 1200, -120, 0, lisse_col) # Lisse AR

        # POSITIONNEMENT DE PART ET D'AUTRE
        # Palette 1 : Tout à gauche
        draw_real_pallet(f2, 0, w_p, 1200, 0)
        draw_box(f2, 10, w_p-10, 10, 1190, 150, 150+(nb_c*item['H']), "rgba(30, 136, 229, 0.5)")
        
        # Palette 2 (si possible) : Tout à droite
        if nb_pal_possibles >= 2:
            x_start_droite = l_lisse - w_p
            draw_real_pallet(f2, x_start_droite, w_p, 1200, 0)
            draw_box(f2, x_start_droite+10, l_lisse-10, 10, 1190, 150, 150+(nb_c*item['H']), "rgba(30, 136, 229, 0.5)")
            
        # Palette 3 (si possible, ex: lisse 3600) : Au centre
        if nb_pal_possibles >= 3:
            x_center = (l_lisse / 2) - (w_p / 2)
            draw_real_pallet(f2, x_center, w_p, 1200, 0)
            draw_box(f2, x_center+10, x_center+w_p-10, 10, 1190, 150, 150+(nb_c*item['H']), "rgba(30, 136, 229, 0.5)")

        f2.update_layout(scene=dict(aspectmode='data'), height=650, margin=dict(l=0,r=0,b=0,t=0))
        st.plotly_chart(f2, use_container_width=True)

with tabs[1]:
    st.subheader("Instructions de pose (2D)")
    ...
    def fig2d(plan, col, title):
        ...
        f.update_layout(title=title, yaxis=dict(scaleanchor="x"), showlegend=False, height=450)
        return f
    c2d1.plotly_chart(fig2d(plan_a, "#1E88E5", "Couche IMPAIRE (Base)"), use_container_width=True)
    c2d2.plotly_chart(fig2d(plan_b, "#E53935", "Couche PAIRE (Verrouillage)"), use_container_width=True)

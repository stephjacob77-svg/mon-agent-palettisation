...
# --- CONFIGURATION ---
st.set_page_config(page_title="Expert WMS Pro v9.4", layout="wide")
...
def draw_real_pallet(fig, x_off, w_p, l_p, z_off):
    c_wood = "#8D6E63"
    for sx in [0, w_p/2-50, w_p-100]: draw_box(fig, x_off+sx, x_off+sx+100, 0, l_p, z_off, z_off+25, c_wood)
    for dx in [0, w_p/2-50, w_p-100]:
        for dy in [0, l_p/2-50, l_p-100]: draw_box(fig, x_off+dx, x_off+dx+100, dy, dy+100, z_off+25, z_off+125, c_wood)
    draw_box(fig, x_off, x_off+w_p, 0, l_p, z_off+125, z_off+150, c_wood)

# --- UI ---
st.sidebar.title("🏭 Expert WMS v9.4")
fmt_pal = st.sidebar.radio("Format Palette", ["800x1200 (Europe)", "1000x1200 (VMF)"])
ref_sel = st.sidebar.selectbox("Article", st.session_state.db_refs["Référence"].tolist())
h_rack_max = st.sidebar.number_input("Hauteur Max Alvéole (mm)", value=1800)
...
h_pal = 150 + (nb_c * item['H'])

st.header(f"📦 Rapport Haute Visibilité : {ref_sel}")
c1, c2, c3 = st.columns(3)
c1.metric("Colis / Couche", len(plan_a))
c2.metric("Poids Total", f"{int(len(plan_a)*nb_c*item['P'])} kg")
c3.metric("Garde d'air", f"{h_rack_max - h_pal} mm")

t1, t2 = st.tabs(["🏗️ Rendu 3D Avancé", "📋 Plans 2D & Étiquette"])

with t1:
    v1, v2 = st.columns(2)
    with v1:
        st.subheader("Structure Palette")
        f1 = go.Figure()
    ...
    with v2:
        st.subheader("Positionnement Rack")
        f2 = go.Figure()
        # Rack Structure
        for px in [-100, 2700]:
            for py in [0, 1100]: draw_box(f2, px, px+100, py, py+100, -150, h_rack_max+150, "royalblue")
        draw_box(f2, 0, 2700, 0, 100, -150, 0, "orange") # Lisse AV
        draw_box(f2, 0, 2700, 1100, 1200, -150, 0, "orange") # Lisse AR
        for i in range(int(2700 // w_p)):
            x_pos = i * (w_p + 50) + 25
            draw_real_pallet(f2, x_pos, w_p, 1200, 0)
            draw_box(f2, x_pos+10, x_pos+w_p-10, 10, 1190, 150, h_pal, "rgba(33, 150, 243, 0.4)")
...
with t2:
    ...
    def fig2d(plan, col, title):
        ...
        return f
    c2d1.plotly_chart(fig2d(plan_a, "#1E88E5", "Couche IMPAIRE (A)"), use_container_width=True)
    c2d2.plotly_chart(fig2d(plan_b, "#E53935", "Couche PAIRE (B)"), use_container_width=True)
    st.markdown(f"""<div style="border:4px solid black; padding:15px; background:white; color:black; font-family:monospace;">
        <h2>PALETTE : {ref_sel}</h2><hr>
        <b>POIDS : {int(len(plan_a)*nb_c*item['P'])} KG | COLIS : {len(plan_a)*nb_c}</b><br>
        <b>HAUTEUR : {h_pal} MM | SUPPORT : {fmt_pal}</b></div>""", unsafe_allow_html=True)

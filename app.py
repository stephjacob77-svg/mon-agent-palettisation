...
# --- CONFIGURATION ---
st.set_page_config(page_title="WMS Master Config v10.4", layout="wide")
...
# --- FONCTIONS GRAPHIQUES 3D (Palette uniquement) ---
def draw_box_3d(fig, x0, x1, y0, y1, z0, z1, color, opacity=0.8):
    fig.add_trace(go.Mesh3d(x=[x0, x1, x1, x0, x0, x1, x1, x0], y=[y0, y0, y1, y1, y0, y0, y1, y1], z=[z0, z0, z0, z0, z1, z1, z1, z1],
        i=[7,0,0,0,4,4,6,6,4,0,3,2], j=[3,4,1,2,5,6,5,2,0,1,6,3], k=[0,7,2,3,6,7,1,1,5,5,7,6], color=color, opacity=opacity, flatshading=True))
...
with t_config:
    ...
    with c3:
        ...
        h_list = [st.number_input(f"Niveau {i} (0=Sol)", 1000, 4000, 2000 if i==0 else 1800, key=f"h{i}") for i in range(st.session_state.nb_niv + 1)]
...
with t_sim:
    if len(st.session_state.catalogue) > 0:
        ...
        for h_max in st.session_state.h_list:
            ...
            results.append({"c": max_c, "p": p_tot, "air": h_max - h_pal, "fill": (h_pal/h_max)*100})

        # --- AFFICHAGE ---
        col_left, col_right = st.columns([1, 2])

        with col_left:
            st.subheader("📦 Focus Palette")
            # Vue 3D Palette Seule
            fig_p3d = go.Figure()
            draw_box_3d(fig_p3d, 0, w_p, 0, 1200, 0, 150, "#8D6E63") # Socle
            for k in range(results[0]['c']):
                p = plan_base if k % 2 == 0 else plan_cross
                for b in p:
                    draw_box_3d(fig_p3d, b['x'], b['x']+b['w'], b['y'], b['y']+b['h'], 150+(k*item['H']), 150+((k+1)*item['H']), "#1E88E5" if k%2==0 else "#E53935")
            fig_p3d.update_layout(scene=dict(aspectmode='data'), height=400, margin=dict(l=0,r=0,b=0,t=0))
            ...
            def plot_2d(plan, title, color):
                ...
                f.update_layout(title=title, xaxis=dict(scaleanchor="y"), height=300, showlegend=False, margin=dict(l=10,r=10,t=30,b=10))
                return f
        ...
        with col_right:
            st.subheader("📐 Élévation Face (Rack détaillé)")
            fig_2d = go.Figure()
            
            z_accum = 0
            for i, res in enumerate(results):
                # Échelles (Montants)
                fig_2d.add_rectangle(x0=-80, x1=0, y0=z_accum, y1=z_accum+st.session_state.h_list[i], fillcolor="#455A64")
                fig_2d.add_rectangle(x0=l_val, x1=l_val+80, y0=z_accum, y1=z_accum+st.session_state.h_list[i], fillcolor="#455A64")
                
                # Lisses (Sauf sol)
                if i > 0:
                    fig_2d.add_rectangle(x0=0, x1=l_val, y0=z_accum-100, y1=z_accum, fillcolor="orange")
                
                # Palettes
                gap = (l_val - (nb_pal_l * w_p)) / (nb_pal_l + 1)
                for j in range(nb_pal_l):
                    px = gap + j*(w_p + gap)
                    # Dessin du bloc palette
                    h_p = 150 + (res['c'] * item['H'])
                    fig_2d.add_rectangle(x0=px, x1=px+w_p, y0=z_accum, y1=z_accum+h_p, fillcolor="#90CAF9", line_color="blue")
                    # Ligne de séparation bois
                    fig_2d.add_hline(y=z_accum+150, line_dash="dot", line_color="brown")

                # Annotations Techniques
                fig_2d.add_annotation(x=l_val/2, y=z_accum + h_p + (res['air']/2), text=f"GA: {res['air']}mm", showarrow=False, font=dict(color="green", size=10))
                fig_2d.add_annotation(x=l_val+150, y=z_accum + (st.session_state.h_list[i]/2), 
                                      text=f"Niv {i}: {res['fill']:.1f}% remp.<br>{int(res['p'])}kg", align="left", showarrow=False)
                
                z_accum += st.session_state.h_list[i]

            fig_2d.update_layout(xaxis=dict(range=[-200, l_val+600], title="Largeur (mm)"), yaxis=dict(title="Hauteur (mm)"), height=800, plot_bgcolor="white")
            st.plotly_chart(fig_2d, use_container_width=True)

            # --- CONSEILS D'OPTIMISATION ---
            st.success("💡 **Analyse d'optimisation :**")
            opti_cols = st.columns(2)
            if results[0]['air'] > item['H']:
                opti_cols[0].write(f"👉 **Gagner un niveau ?** En réduisant les lisses de {int(results[0]['air']-100)}mm, vous pourriez ajouter un étage complet.")
            if results[0]['p'] < st.session_state.capa * 0.6:
                opti_cols[1].write("👉 **Densité :** Votre charge est légère. Vous pouvez augmenter la hauteur des palettes sans risque pour les lisses.")
